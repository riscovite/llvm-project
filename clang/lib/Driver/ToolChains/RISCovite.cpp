//===--- RISCovite.cpp - RISCovite ToolChain Implementations ----*- C++ -*-===//
//
...
void riscovite::Linker::ConstructJob(Compilation &C, const JobAction &JA,
                                   const InputInfo &Output,
                                   const InputInfoList &Inputs,
                                   const ArgList &Args,
                                   const char *LinkingOutput) const {
  const auto &ToolChain = static_cast<const RISCovite &>(getToolChain());
...
void riscovite::StaticLibTool::ConstructJob(Compilation &C, const JobAction &JA,
                                          const InputInfo &Output,
                                          const InputInfoList &Inputs,
                                          const ArgList &Args,
                                          const char *LinkingOutput) const {
...
/// RISCovite - RISCovite tool chain which can call as(1) and ld(1) directly.

RISCovite::RISCovite(const Driver &D, const llvm::Triple &Triple,
                 const ArgList &Args)
  ...
  if (!D.SysRoot.empty()) {
    ...
    llvm::sys::path::append(P, Triple.getTriple());
...
std::string RISCovite::ComputeEffectiveClangTriple(const ArgList &Args,
                                                 types::ID InputType) const {
  llvm::Triple Triple(ComputeLLVMTriple(Args, InputType));
  return Triple.str();
}

Tool *RISCovite::buildLinker() const { return new tools::riscovite::Linker(*this); }

Tool *RISCovite::buildStaticLibTool() const {
  return new tools::riscovite::StaticLibTool(*this);
}

ToolChain::RuntimeLibType
RISCovite::GetRuntimeLibType(const ArgList &Args) const {
...
ToolChain::CXXStdlibType RISCovite::GetCXXStdlibType(const ArgList &Args) const {
...
void RISCovite::addClangTargetOptions(const ArgList &DriverArgs,
...
void RISCovite::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
...
void RISCovite::AddClangCXXStdlibIncludeArgs(const ArgList &DriverArgs,
                                           ArgStringList &CC1Args) const {
...
void RISCovite::AddCXXStdlibLibArgs(const ArgList &Args,
...
SanitizerMask RISCovite::getSupportedSanitizers() const {
  SanitizerMask Res = ToolChain::getSupportedSanitizers();
  return Res;
}

SanitizerMask RISCovite::getDefaultSanitizers() const {
  SanitizerMask Res;
